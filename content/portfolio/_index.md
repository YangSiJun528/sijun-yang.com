...
Java와 Kotlin 기반의 Spring Boot를 주요 스택으로, AWS 환경에서 백엔드를 개발해 왔습니다.<br>
...
개인 프로젝트와 오픈소스에서도 실제 사용자가 있는 서비스를 만들고 개선해 왔습니다.
...
하지만 이 과정을 하나의 HTTP 요청에서 처리했고, 이로 인해 다음 문제가 존재했습니다.

1. 중간에 실패해도 이미 완료된 외부 게시까지 함께 되돌릴 수 없어, 전체 작업을 재시도하면 중복 게시가 발생할 수 있었습니다.
2. 관리자 화면에는 최종 성공 여부만 표시돼, 어느 단계에서 왜 실패했는지 알 수 없었습니다.

그렇기에 운영자는 상태 확인과 실패 원인 파악, 복구를 매번 개발자에게 요청해야 했습니다.
...
또한 Instagram API가 제공하는 멱등 기능을 적용해, 복구 과정에서 중복 게시가 발생하지 않도록 했습니다.
...
기존 CloudWatch 기반 환경에서 쉽게 도입할 수 있고 익숙한 SQL을 제공해 주어 Athena를 선택했습니다.
...
- 친구 추천 쿼리: `OR` 조건을 `UNION ALL`로 변경, 쿼리 실행 시간 3,020ms → 61ms
...
기존 채팅 로직은 실행 환경만 옮겨 변경 범위를 줄이고, 실시간 이벤트 전달은 별도의 Live 서버로 분리했습니다.      
...
Hello, GSM은 광주소프트웨어마이스터고등학교의 신입생 모집에 사용되는 입학 지원 서비스입니다.<br>기존 시스템의 복잡한 과정과 불편한 UX를 개선하기 위해 2022년부터 교내 개발팀이 직접 개발/운영하고 있습니다.
